...
void backward_local_layer_gpu(local_layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        for(j = 0; j < locations; ++j){
            ...
            gemm_offset_gpu(1,0,m,n,k,1,a,i*l.outputs+j,locations,b,j,locations,1,c,j*l.size*l.size*l.c*l.n,n);
        }
        ...
        if(net.delta_gpu.mem){
            for(j = 0; j < locations; ++j){
                ...
                gemm_offset_gpu(0,1,m,n,k,1,a,j*l.size*l.size*l.c*l.n,m,b,i*l.outputs+j,locations,0,c,j,locations);
            }
