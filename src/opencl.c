...
void opencl_init(int *gpus, int ngpus) {
    ...
    for (d = 0; d < ngpus; ++d) {
        ...
        clGetDeviceInfo(opencl_devices[opencl_device_id_t], CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE, sizeof(cl_uint), &native_double_width, NULL);
        printf("Device double precision: %s\n", native_double_width == 0 ? "NO" : "YES");
